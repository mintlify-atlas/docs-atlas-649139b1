...
<Note>
  `-q` is a shorthand that sets both `-w rockyou.txt` and `-r best64.rule` simultaneously. Any `-w` or `-r` flags specified alongside `-q` are overridden by the quick-mode defaults because `-q` is processed last in the argument loop.
</Note>
